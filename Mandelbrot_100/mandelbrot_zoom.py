...
w, h = 1024, 1024
mfactor = 0.5
bar = Bar("Processing: ", max=h)

def Mandelbrot(x, y, max_iteration, minx, maxx, miny, maxy):
    zx = 0
    zy = 0
    RX1, RX2, RY1, RY2 = px-R/2, px+R/2, py-R/2, py+R/2  #zoom-in variables
...
def gen_Mandelbrot_image(sequence):
    bitmap = Image.new("RGB", (w, h), "white")   #try blue
    pix = bitmap.load()
    for x in range(w):
        for y in range(h):
            ...
            b = min(255, round(g*255))
            pix[x,y] = int(r) + (int(g) << 8) + (int(b) << 16)
        bar.next()
        if x % 5 ==0:
            print(f"{x/w:.2}", end= " ")
        bitmap.save("Mandelbrot"+str(sequence)+".jpg")
    bar.finish()
